...
let today = <Fragment>
    ...
    &nbsp;
    <Text style={{color:"gray"}}>Today</Text> <Text> {moment().format('MMMM Do')}</Text>
</Fragment>
            ...
            <View style={{flexDirection:"row", flexWrap: "wrap", justifyContent:"center", alignItems: "center", width: "100%"}}>

                {
                    infoBadgesData.map(({value, iconName, label}, index)=>{
                        ...
                        return <InfoBadge key={index} value={value} iconName={iconName} label={label} />
                    })
                }
...
const InfoBadge = ({label, value, iconName}) => {
    return (
        <View style={{
            ...
            <Text style={{fontSize:15, fontWeight:"bold", color:"#6b6565"}}>{label}</Text>
